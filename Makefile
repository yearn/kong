...
dev:
	...
	# Run commands
	@tmux send-keys -t kongdevenv:0.0 'PORT=$${PORT:-3001} bun run --filter web dev' C-m
	@tmux send-keys -t kongdevenv:0.1 'bun run --filter ingest dev' C-m
	@tmux send-keys -t kongdevenv:0.2 'bun run --filter terminal dev' C-m
