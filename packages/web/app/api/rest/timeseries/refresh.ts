...
import { getRecentTimeseries, getTranches, getTrancheControllers, getVaults, TimeseriesRow } from './db'
  ...
  for (const { addresses, labels } of scoped) {
    for (const { chainId, address } of addresses) {
      for (const { label } of labels) {
        const rows: TimeseriesRow[] = await getRecentTimeseries(chainId, address, label)

        scopedPairs.push([
          getTimeseriesLatestKey(label, chainId, address.toLowerCase()),
          JSON.stringify({
            value: rows.map(row => ({
              time: Number(row.time),
              component: row.component,
