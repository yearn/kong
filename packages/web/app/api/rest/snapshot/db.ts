...
function getSnapshotEstimatedAprLabel(snapshot: VaultSnapshot): string | undefined {
...
async function hydrateStrategyEstimatedApr(
  ...
  const rows = await getLatestEstimatedAprRows(db, chainId, vaultAddress, {
    label: getSnapshotEstimatedAprLabel(snapshot),
    includeAddresses: strategies,
  })
