...
import {
  ...
  extractFees__v2,
  extractFees__v3,
  extractLockedProfit__v2,
  extractLockedProfit__v3,
} from './apy'
...
describe('abis/yearn/lib/apy', function () {
  ...
  afterAll(async function () {
    await db.query('DELETE FROM evmlog WHERE address = ANY($1)', [
      [addresses.v2.yvusdt, addresses.v3.yvusdca],
    ])
  })
  ...
  it('extracts v2 fees', async function () {
    ...
    const fees = await extractFees__v2(
      mainnet.id,
      addresses.v2.yvusdt,
      strategies,
      15871070n
  ...
  it('extracts v2 locked profit', async function () {
    ...
    const noLockedProfit = await extractLockedProfit__v2(
      mainnet.id,
      addresses.v2.yvusdt,
      18965226n
    )
  ...
  it('yvUSDT 0.4.3 @ block 15871070', async function () {
    ...
    const strategies: `0x${string}`[] = [
      ...
      '0xd8F414beB0aEb5784c5e5eBe32ca9fC182682Ff8',
    ]
    const yvusdt = ThingSchema.parse({
      chainId: 1,
      address: addresses.v2.yvusdt,
  ...
  it('extracts v3 vault fees', async function () {
    ...
    const fees = await extractFees__v3(
      polygon.id,
    ...
    expect(fees.management).to.eq(0)
    expect(fees.performance).to.eq(0.1)
  })
  ...
  it('extracts v3 locked profit', async function () {
    ...
    const noLockedProfit = await extractLockedProfit__v3(
      polygon.id,
      addresses.v3.yvusdca,
      49181585n
    )
