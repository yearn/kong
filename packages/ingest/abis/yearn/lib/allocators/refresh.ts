...
async function run() {
  try {
    ...
    for (const row of result.rows) {
      ...
      const outcome = update ? (update.applied ? 'applied' : update.staleUpdated ? 'stale' : 'skipped') : 'dry_run'
      console.log(JSON.stringify({ chainId, vault: row.address, address: reported.address, support: reported.support,
        revision: reported.revision, asOfBlock: reported.asOfBlock, stale: reported.stale, lastError: reported.lastError, outcome, written: Boolean(update?.applied || update?.staleUpdated) }))
    }
