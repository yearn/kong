...
export async function add(job: Job, data: any, options?: any) {
  ...
  if (!queues[queue]) {
    queues[queue] = connect(queue);
  }
  await queues[queue].add(job.name, data, {
...
export function workers(
  ...
  for (const chain of chains) {
    result.push(worker(`${queueSuffix}-${chain.id}`, handler, chain.id));
  }
  return result;
}
...
export function worker(
  ...
  const timer = setInterval(async () => {
    const MQ_CONCURRENCY_MAX_PER_PROCESSOR_ENVAR = chainId
      ? `MQ_CONCURRENCY_MAX_PER_PROCESSOR_${chainId}`
      : "MQ_CONCURRENCY_MAX_PER_PROCESSOR";
    const MQ_CONCURRENCY_THRESHOLD_ENVAR = chainId
      ? `MQ_CONCURRENCY_THRESHOLD_${chainId}`
      : "MQ_CONCURRENCY_THRESHOLD";
    const MQ_CONCURRENCY_MAX_PER_PROCESSOR = (process.env[
      MQ_CONCURRENCY_MAX_PER_PROCESSOR_ENVAR
    ] || 50) as number;
    const MQ_CONCURRENCY_THRESHOLD = (process.env[
      MQ_CONCURRENCY_THRESHOLD_ENVAR
    ] || 200) as number;

    const jobs = await queue.count();
    const targetConcurrency = computeConcurrency(jobs, {
      min: 1,
      max: MQ_CONCURRENCY_MAX_PER_PROCESSOR,
